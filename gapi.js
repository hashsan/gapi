...
;(function(root){
  ...
  // fetch -> _fetch
  function _fetch(url,obj,use_cache){
    // default no-chache
    // and need .catch()
    obj = obj||{}
    const okOnly = (res) =>{
  ...
  */
  ...
  function base(opt){
    ...
    o.timestamp = async (file)=>{
      ...
      var res = await _fetch(url,{method,headers,body})
      .then(d=>d.json())
      .catch(e=>void 0)
  ...
  function entry(opt){
    ...
    o.data =async (file)=>{
      ...
      return _fetch(download_url).then(d=>d.text())
    }        
  ...
  function wrapentry(url,auth){
    console.log('wrapentry call to entry. gapi -> repo_api')
    var opt ={}
    opt.t1 = auth;
    opt.t2 = ''
    var {owner,repo} = giturlToEnv(url)
    //console.log(owner,repo)
    opt.u = `/repos/${owner}/${repo}/contents/`
    return entry(opt)
